#!/usr/bin/env python3
"""Run DMSTrack's Late Fusion detections through AB3DMOT-style tracking and
score under BOTH our standard evaluator and the V2V4Real protocol.

Killer comparison: DMSTrack's published 'Late Fusion+AB3DMOT (our impl)' AMOTA
...
Inputs come from the cloned DMSTrack repo:
  - Late Fusion detections (KITTI-tracking format):
        DMSTrack/AB3DMOT/data/v2v4real/detection/late_fusion_Car_val/{0000..0008}.txt
  - GT labels:
...
def parse_kitti_det(path: Path) -> Dict[int, List[Dict]]:
    """Parse a Late Fusion KITTI-format det file → {frame_idx: [det_dict, ...]}."""
...
def main():
    ...
    print(f"=== DMSTrack Late Fusion+AB3DMOT through our evaluator ===")
