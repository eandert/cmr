#!/usr/bin/env python3
"""
Sweep baseline R value across all fast maps, both suites.
Uses per-filter sigma_a already locked in filter_config.py.
"""
import re, subprocess, sys, time

baselines = [1.0, 1.2, 1.5, 2.0]
            ...
            for suite in suites:
                ...
                for line in proc.stdout.split('\n'):
                    ...
                    for pattern in ['step_7_baseline:', 'step_7_gpem_linear:',
                        ...
                        if pattern in line:
                            if any(p in line for p in ['akf_', 'pf_', 'bici_']) and not line.strip().startswith('📊 ' + pattern.split(':')[0]):
                                continue
