...
def render_markdown(results: dict[str, dict[str, str | None]]) -> str:
    ...
    lines.append("")
    lines.append("- **Missing overall bin** (mode=static failures): re-run the importer ")
    lines.append("  (`scripts/import_mmdet_error_models.py` for mmdet-derived sensors) so")
    lines.append("  the distributions CSV emits a count-weighted row with")
    lines.append("  `dist_min=0, dist_max=overall_dist_max` per axis. See lines 335-375.")
