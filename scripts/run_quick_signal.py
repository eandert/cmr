#!/usr/bin/env python3
"""
Quick signal test - one 5-minute trial per configuration to verify R=0.5 results.
3 maps × 2 suites = 6 configurations total (~30 minutes wall time).
"""
...
WORKERS = 8
WARMUP = 30
RECORD = 500  # ~5 min per config

total = len(maps) * len(suites)
done = 0
start = time.time()

print(f"Quick signal test with baseline R=0.5")
print(f"Total configurations: {total} (3 maps × 2 suites)")
print(f"Each configuration: 1 trial, warmup={WARMUP}, record={RECORD}")
print(f"Expected total time: ~{total * 5} minutes\n")
...
for map_name in maps:
    for suite in suites:
        ...
        print(f'  Done in {elapsed_total:.0f}m total')

print(f'\nQuick signal test complete. Total time: {(time.time()-start)/60:.1f}m')
